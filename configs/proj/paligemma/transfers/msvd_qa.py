...
r"""PaliGemma transfer to MSVD Video QA.
...
SPLIT_SIZE = {'train': 24670, 'valid': 5107, 'test': 10136}  # 2024-04-28
...
def training_data(res, *, final_split, num_frames=8, stride=None):
  ...
  pp = '|'.join([
      ...
      # ensure_shape is needed for tf to figure out the tensor shape
...
def add_eval(c, res, num_frames=8, stride=None):
  ...
  pp = '|'.join([
      ...
      'reshape([], key="question")|reshape([], key="answer")',
      'strfmt("answer en {question}", outkey="prefix")',
      'strfmt("{id}#{example/video_id}[{clip/start/timestamp}-{clip/end/timestamp}]: {question}", outkey="question_id")',
      combine_and_keep_eval(TEXT_LEN, keep=('question_id', 'answer')),
  ])

  for freq, name, split, first_k_shards, skip_first_eval in [
      (1/8, 'minitrain', 'train', 1, False),  # To gauge memorization.
      (1/4, 'minival', 'valid', 1, False),  # To monitor val progress.
...
def add_eval_pplx(c, res, num_frames=8, stride=None):
  ...
  for name, split, first_k_shards in [
      ('minitrain', 'train', 1),  # To gauge memorization.
      ('minival', 'valid', 1),  # To tune hparams.
  ]:
...
def sweep_best(add, arg=None):
  ...
  add(lr=3e-6, wd=3e-7, total_epochs=1, **bvcc.arg(res=224, **c))
...
def get_config(arg=None):
  """Config for training."""
  c = bvcc.parse_arg(arg, mode='xm', num_frames=8, stride=26, res=224,
