...
r"""PaliGemma transfer to Screen2words.
"""
...
def training_data(res, text_len=24, final_split=False):
  ...
  Args:
    ...
    final_split: Train also on val or not.
  ...
  c.pp = '|'.join([
      ...
      'copy(inkey="image/id", outkey="_id")',
      'choice_no_replacement(inkey="summary", outkey="suffix")',
      combine_and_keep_train(text_len),
  ])
  return c


def add_eval(c, res, text_len=24, **kw):
  """Screen2words evaluator (CIDER)."""
  pp = '|'.join([
      'copy("summary", "captions")',  # GT for evaluator.
      'decode', f'resize({res})', 'value_range(-1, 1)',
      'strfmt("caption en", outkey="prefix")',
  ...
  for name, split in [
      ...
      ('dev', 'dev'),  # To tune hparams
      ('test', 'test'),
  ]:
    c.evals[f'screen2words/{name}'] = dict(
        ...
        log_percent=0.1, tokenizer=TOKENIZER, pp_fn=pp)
    c.evals[f'screen2words/{name}'].update(kw)


def add_eval_pplx(c, res, text_len=24):
  ...
  for name, split in [
    ...
    c.evals[f'screen2words/{name}/pplx'] = dict(
...
def get_config(arg=None):
  ...
  c.model.llm = dict(vocab_size=256_000 + 1024 + 128, dropout=0.3)
...
def metrics(arg=None):  # pylint: disable=unused-argument
  m = ['training_loss']
  for split in ('test', 'dev', 'minitrain'):
    m.append(('epoch', f'screen2words/{split}/cider'))
    m.append(('epoch', f'screen2words/{split}/pplx/avg'))
  return m
