...
r"""PaliGemma transfer to InfoVQA.
"""
...
def training_data(res, final_split, text_len=48):
  ...
  See Colab:
  http://(internal link)#scrollTo=0yIyusCLhdDy
...
def add_eval(c, res, text_len=48, **kw):
  ...
  for freq, name, split in [
      (0.1, 'minitrain', 'train[:5%]'),  # To gauge memorization. ~1.2k samples.
      (0.1, 'minival', 'val'),  # To tune hparams. ~1.2k samples.
      (1.0, 'test', 'test'),  # Also stores predictions for the test set (3.3k).
  ]:
    c.evals[f'infovqa/{name}'] = dict(
        ...
        to_lower=True,
        # Test server expects two fields: 'questionId' and 'answer'.
        out_question_key='questionId', out_answer_key='answer',
        outfile=f'{{workdir}}/infovqa_{name}.json',
        data={**training_data(res, False, text_len).data, 'split': split},
        log_percent=freq, skip_first=freq == 1, tokenizer=TOKENIZER, pp_fn=pp)
    c.evals[f'infovqa/{name}'].update(kw)


def add_eval_pplx(c, res, text_len=48):
  """Perplexity evaluator to test runs before implementing the real deal."""
  c_train = training_data(res, False, text_len)  # Use mostly same settings as training.
  for name, split in [
      ('minitrain', 'train[:5%]'),  # To gauge memorization.
      ('minival', 'val'),  # To tune hparams.
  ]:
    c.evals[f'infovqa/{name}/pplx'] = dict(
...
def sweep_best(add, arg=None):
  ...
  # In InfoVQA: max prefix: 51, suffix: 42, prefix+sep+suffix: 60.
  c = bvcc.parse_arg(arg, mode='xm', final_split=False)
  add(lr=1e-5, wd=1e-6, total_epochs=3, **bvcc.arg(res=224, **c))
  add(lr=1e-5, wd=1e-6, total_epochs=3, **bvcc.arg(res=448, **c))
  add(lr=3e-6, wd=3e-7, total_epochs=3, **bvcc.arg(res=896, **c))
...
def get_config(arg=None):
  """Config for training."""
  c = bvcc.parse_arg(arg, mode='xm', res=896, text_len=48, final_split=False)

  c.input = training_data(c.res, c.final_split, c.text_len)

  # Instead of epochs, you can also use `total_examples` or `total_steps`.
  c.total_epochs = 3
  c.input.batch_size = {224: 256, 448: 128, 896: 32}[c.res]
  c.optax_name = 'scale_by_adam'
  c.optax = dict(b2=0.999)
  c.lr = 3e-6
  c.wd = 3e-7
  c.grad_clip_norm = 1.0
  c.label_smoothing = 0.4
  c.schedule = dict(decay_type='cosine', warmup_percent=0.05)

  # Add evaluators.
  c.evals = {}
  add_eval(c, c.res, c.text_len, batch_size=256)
...
def metrics(arg=None):  # pylint: disable=unused-argument
  ...
  for split in ('minival', 'minitrain'):
    m.append(f'infovqa/{split}/anls')
    m.append(f'infovqa/{split}/acc')
    m.append(f'infovqa/{split}/pplx/avg')
  return m
