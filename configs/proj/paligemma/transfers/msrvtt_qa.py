...
r"""PaliGemma transfer to MSRVTT Video QA.
...
SPLIT_SIZE = {'train': 114080, 'valid': 7936, 'test': 51680}
...
def add_eval_pplx(c, res, num_frames, stride):
  ...
  for name, split, first_k_shards in [
      ...
      ('minival', 'valid', 2),  # To tune hparams.
  ]:
    c.evals[f'msrvtt_qa/{name}/pplx'] = dict(
...
def sweep_best(add, arg=None):
  ...
  add(lr=1e-5, wd=0.0, total_epochs=1, **bvcc.arg(freeze_vit=True, res=224, **c))
...
def get_config(arg=None):
  ...
  c = bvcc.parse_arg(arg, mode='xm', num_frames=16, stride=9, res=224,
                     freeze_vit=True, freeze_llm=False, final_split=False)
