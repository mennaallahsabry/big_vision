...
r"""PaliGemma transfer to RSVQA-HR.
"""
...
def training_data(res, *, final_split, text_len=32):
  ...
  c.data = dict(
      name='rsvqa_hr/nonum' if ONLY_NON_NUMERIC_ANSWERS else 'rsvqa_hr/all',
...
def add_eval(c, res, text_len=32, **kw):
  ...
  for freq, name, split in [
      ...
      (1.0, 'minival', 'val'),  # Very slow (large)
      (1.0, 'test', 'test'),  # Very slow (large)
      (1.0, 'test2', 'test_2'),  # Very slow (large)
  ]:
    c.evals[f'rsvqa_hr/{name}'] = dict(
        type='proj.paligemma.transfers.rsvqa',
        pred='decode', pred_kw={'max_decode_len': text_len},
        # Answers in reference evaluation are converted to lower case.
        # See https://rrc.cvc.uab.es/?ch=17&com=tasks or paper.
        to_lower=True,
        outfile=f'{{workdir}}/rsvqa_hr_{name}.json',
        data={**c_train.data, 'split': split},
        log_percent=freq, skip_first=freq == 1, tokenizer=TOKENIZER, pp_fn=pp)
    c.evals[f'rsvqa_hr/{name}'].update(kw)
...
def add_eval_pplx(c, res, text_len=32):
  ...
  for name, split in [
    ...
    c.evals[f'rsvqa_hr/{name}/pplx'] = dict(
...
def sweep_best(add, arg=None):
  ...
  add(lr=1e-5, wd=0.0, total_epochs=1, **bvcc.arg(res=224, **c))
  add(lr=1e-5, wd=0.0, total_epochs=1, **bvcc.arg(res=448, **c))
  # 896 not better: (internal link)
  # add(lr=1e-5, wd=0.0, total_epochs=1, **bvcc.arg(res=896, **c))
...
def metrics(arg=None):  # pylint: disable=unused-argument
  m = ['training_loss']
  for split in ('minival', 'test', 'test2', 'minitrain'):
    m.append(f'rsvqa_hr/{split}/acc_any')
    m.append(f'rsvqa_hr/{split}/acc_avg')
    m.append(f'rsvqa_hr/{split}/acc_avg_nonum')
    m.append(f'rsvqa_hr/{split}/anls')
    m.append(f'rsvqa_hr/{split}/pplx/avg')
  return m
