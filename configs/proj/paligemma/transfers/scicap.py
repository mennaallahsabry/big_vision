...
r"""PaliGemma transfer to SciCap.
"""

import big_vision.configs.common as bvcc
from big_vision.configs.proj.paligemma.transfers.common import combine_and_keep_train, combine_and_keep_eval, TOKENIZER

# This feature name is also hardcoded in the coco_captions evaluator.
CAPTION_FEATURE = 'caption/lowercase_and_token_and_remove_figure_index'


def training_data(res, *, final_split, text_len=96):
  ...
  Args:
    res: The requested image resolution (e.g. 224).
  ...
  c.data = dict(
      name='scicap/first_sentence_subfig_no',
      split='train+val' if final_split else 'train',
  )
  c.pp = '|'.join([
      'decode', f'resize({res})', 'value_range(-1, 1)',
      'strfmt("caption en", outkey="prefix")',
      f'copy(inkey="{CAPTION_FEATURE}", outkey="suffix")',
      combine_and_keep_train(text_len),
  ])
  return c


def add_eval(c, res, text_len=96, **kw):
  """SciCap evaluator (BLEU-4)."""
  pp = '|'.join([
      f'reshape([1], inkey="{CAPTION_FEATURE}", outkey="captions")',  # GT for evaluator.
      'decode', f'resize({res})', 'value_range(-1, 1)',
      'strfmt("caption en", outkey="prefix")',
  ...
  for name, split in [
      ...
      ('val', 'val'),  # To tune hparams
      ('test', 'test'),
  ]:
    c.evals[f'scicap/{name}'] = dict(
        ...
        log_percent=0.3, tokenizer=TOKENIZER, pp_fn=pp)
    c.evals[f'scicap/{name}'].update(kw)


def add_eval_pplx(c, res, text_len=96):
  ...
  for name, split in [
    ...
    c.evals[f'scicap/{name}/pplx'] = dict(
...
def sweep_best(add, arg=None):
  ...
  # 896 is too slow and not better than 448: (internal link)
...
def metrics(arg=None):  # pylint: disable=unused-argument
  m = ['training_loss']
  for split in ('test', 'val', 'minitrain'):
    m.append(f'scicap/{split}/pplx/avg')
    m.append(f'scicap/{split}/cider')
  return m
