...
r"""PaliGemma transfer to Scene Text VQA (ST-VQA).
"""
...
def add_eval(c, res, text_len=48, **kw):
  ...
  for freq, name, split in [
      (0.1, 'minitrain', 'train[:5%]'),  # To gauge memorization. ~2.5k samples.
      (0.1, 'minival', 'val'),  # Pseudo-test.
      (1.0, 'test', 'test'),  # Also stores predictions for the test set (4.1k).
  ]:
    c.evals[f'stvqa/{name}'] = dict(
        type='proj.paligemma.transfers.vqa',
        pred='decode', pred_kw={'max_decode_len': text_len},
        # Answers in reference evaluation are converted to lower case.
        # See https://rrc.cvc.uab.es/?ch=17&com=tasks or paper.
        to_lower=True,
        outfile=f'{{workdir}}/stvqa_{name}.json',
        data={**training_data(res, False, text_len).data, 'split': split},
        log_percent=freq, skip_first=freq == 1, tokenizer=TOKENIZER, pp_fn=pp)
    c.evals[f'stvqa/{name}'].update(kw)
...
def add_eval_pplx(c, res, text_len=48):
  ...
  for name, split in [
    ...
    c.evals[f'stvqa/{name}/pplx'] = dict(
...
def get_config(arg=None):
  """Config for training."""
  # max prefix: 29, suffix: 50, prefix+sep+suffix: 59
...
def metrics(arg=None):  # pylint: disable=unused-argument
  ...
  for split in ('minival', 'minitrain'):
    m.append(f'stvqa/{split}/anls')
    m.append(f'stvqa/{split}/acc')
    m.append(f'stvqa/{split}/pplx/avg')
  return m
