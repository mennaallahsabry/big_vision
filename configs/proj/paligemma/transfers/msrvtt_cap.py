...
r"""PaliGemma transfer to MSRVTT Video captioning.
...
SPLIT_SIZE = {'train': 4663, 'valid': 316, 'test': 2094, 'test_v2': 2271}
...
def add_eval(c, res, num_frames, stride):  # pylint: disable=unused-argument
  ...
  pp = '|'.join([
      ...
      f'video_ensure_shape("image", {(num_frames, res, res, 3)})',
      'strfmt("caption en", outkey="prefix")',
      'strfmt("{example/video_id}[{clip/start/timestamp}-{clip/end/timestamp}]", outkey="image/id")',
...
def add_eval_pplx(c, res, num_frames, stride):
  ...
  for name, split, first_k_shards in [
    ...
    c.evals[f'msrvtt/{name}/pplx'] = dict(
...
def sweep_best(add, arg=None):
  ...
  add(lr=1e-5, wd=0.0, total_epochs=20, **bvcc.arg(res=224, freeze_vit=True, **c))
...
def get_config(arg=None):
  """Config for training."""
  c = bvcc.parse_arg(arg, mode='xm', num_frames=16, stride=9, res=224,
